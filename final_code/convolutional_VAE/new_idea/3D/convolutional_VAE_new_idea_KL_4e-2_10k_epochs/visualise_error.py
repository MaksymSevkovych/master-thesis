...
def compute_errors(
    model: torch.nn.Module, data_loader: DataLoader, p: float = 2
) -> dict:
    images = []
    ...
    for index, imgs in enumerate(images):
        ...
        for img in tqdm(imgs):
            ...
            recon = model.decoder(
                model.sample([digit], mu[0].detach(), log_var[0].detach())[2].unsqueeze(
                    0
                )
            )
...
if __name__ == "__main__":
    ...
    plot_errors(compute_errors(model, data_loader, p=2))
