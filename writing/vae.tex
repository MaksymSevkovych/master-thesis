...
Considering the approximate posterior $q_{\phi}$ leads to the following consideration of the $\log$ of the evidence $p(x)$
...
\end{align*}
The next step is to consider the linearity of the integral and the fact that holds $\log (ab) = \log a +\log b$, which yields
\begin{align*}
...
Due to the fact that $q_{\phi} (z|x)$ and $p(\e)$ are both probability density functions, this leads to
\begin{align*}
...
\Require $p(z|x) \sim \mathcal{N}(\m_p,\S_p)$ \Comment{Declare a $k$-dimensional true posterior density.}
\Require $q(z|x) \sim \mathcal{N}(\m_q,\S_q)$ \Comment{Declare a $k$-dimensional approximative posterior density.}
\For{epoch in epochs}
	\For{image in batch}
	    \State $\m, \S$ = encoder(image) \Comment{Encode the image onto mean and variance.}
	    \State $z\sim q(z|x)$ \Comment{Sample from approximative posterior density.}
		\State reconstructed = decoder($z$) \Comment{Decode the generated sample.}
    	\State KL-divergence = $\kldiv{q(z|x)}{p(z|x)}$ \Comment{Compute the regularisation loss.}
    	\State $\loss_{\text{rec}} = $ MSE(reconstructed, image) \Comment{Compute the reconstruction loss.}
    	\State loss = $\loss_{\text{rec}} + \l\cdot$KL-divergence \Comment{Compute the overall loss.}
...
If we now take a look at the reconstructions of the variational autoencoder, see Figure \ref{fig:convolutional_VAE_snd_KL_4e-4_10k_epochs_3D_inference}, we see that they still are not good, even though they are a little smoother than compared to the $\l = \num{4e-5}$ case, see Figure \ref{fig:convolutional_VAE_snd_KL_4e-5_10k_epochs_3D_inference}.
