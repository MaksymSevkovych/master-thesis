...
Let the chosen optimizer be AMSGrad with a learning rate $\g>0$ and Kullback-Leibler coefficient $\l\in[0,\infty)$. Furthermore, let the chosen loss function be the MSE loss function. Then the training of a variational autoencoder looks as follows.
\caption{Variational Autoencoder}\label{alg:general_vae}
\begin{algorithmic}[1]
\Require $\g \gets \num{3e-4}$
\Require $\l \gets \l\in[0,\infty)$
\For{epoch in epochs}
	\For{image in batch}
	    \State $\m, \s$ = encoder(image) \Comment{Encode the image onto mean and variance.}
	    \State $z\sim \mathcal{N}(\m, \s)$ \Comment{Sample from encoded distribution.}
	    ...
	    \State optimization(loss, $\g$, $\l$) \Comment{Perform an optimization step.}
    \EndFor
\EndFor
\end{algorithmic}
\end{algorithm}
