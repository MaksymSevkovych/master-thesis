...
\begin{definition}
Let $L\in\N$, $\f_1,\ldots,\f_L$ be activation functions and $H_1, \ldots, H_L$ be $\f_i$-layers with parameters $\t_i = (W_i, b_i)\in\T$ for all $i \in \{1,\ldots,L\}$, where $\T$ is a \textbf{parameter space}. Furthermore, let  $\t = (\t_1, \ldots \t_L)$, $\f = (\f_1, \ldots, \f_L)$ and $d_1,\ldots,d_{L+1}\in\N$.\\
...
Let $d, n \in \N$ and $X\subseteq \R^d$ and $Y \subseteq \R^n$, that we will refer to as input and output space. Furthermore, let $t\in\R^n$ be a prediction for $y\in Y$.\\
...
\item[\textbf{Squared Error Loss}] $L(y, t) = \left|y - t\right|^2$,
\item[\textbf{Linear Error Loss}]  $L(y, t) = \left|y - t\right|$.
\end{mydescription}
...
\begin{mydescription}{\widthof{\textbf{Squared Error Loss}}}
\item[\textbf{Squared Error Loss}] $L(y, t) = \sum_{i=1}^n\left|y_i - t_i\right|^2$,
\item[\textbf{Linear Error Loss}]  $L(y, t) = \sum_{i=1}^n\left|y_i - t_i\right|$,
\end{mydescription}
...
Let $X,Y$ be arbitrary input and output spaces, $L$ be a loss function and $\prob$ be a probability measure on $X \times Y$.\\
...
since the inverse operator $U^{-1}$ exists (because the eigenvalue $\lambda = 0$ does not belong to the spectrum, see \cite[theorem~IX.5.3]{kantorovich2016functional}), there exists one unique solution to \ref{eq:lfe}, for each $y\in\H$. Now, we define the functional
...
+ \left\langle Ux^{\ast}, x^{\ast}\right\rangle
&= \left\langle Ux^{\prime}, x^{\prime}\right\rangle - \left( \left\langle Ux^{\prime}, x^{\ast}\right\rangle + \left\langle Ux^{\ast}, x^{\prime}\right\rangle \right)	+ \left\langle Ux^{\ast}, x^{\ast}\right\rangle
\end{align*}
...
\end{align*}
Thus the direction of steepest descent at $x^{(0)}\in\H$ is given by $z_1 = Ux^{(0)} - y$.\\
Lastly, to determine the descent value we consider the equation
\begin{align*}
...
The gradient descent algorithm with parameters as asserted in \ref{cor:gd} does converge, as we have seen in the proof. However, one might pose the question how quickly it converges. This question we want to consider in the following theorem.
...
\end{align}
Considering the inequality $F(x^{\prime}_1) \geq F(x^{(1)})$, which holds due to the fact that $x_1$ is the iterate performed with optimal descent value (\textcolor{red}{at least the author proposes? Feels somewhat awkward though}), leads to
\begin{align*}
...
\begin{align*}
\left\|V\left(x^{\prime}_1 - x^{\ast}\right)\right\| &= \left\| TV(x^{(0)} - x^{\ast})\right\|\\
&\leq \left\| T \right\| \left\|V(x^{(0)} - x^{\ast})\right\|\\
...
\begin{align*}
\risk_{L, D} (f) = \frac{1}{k} \sum_{i=1}^{k} L\left(x_i, y_i, f(x_i)\right) = \int_{D} L(x,y,f(x)) d(x,y).
\end{align*}
If we now randomly choose a subset $B \subseteq D$ of length $b\in\{1,\ldots,n\}$ which we will call \textbf{mini-batch of size $b$}, we can approximate the empirical risk $\risk_{L, D} (f)$ through $\risk_{L, B} (f)$.
\begin{align*}
\risk_{L,B}(f) = \int_{B}L(x, y, f(x)) d(x,y).
\end{align*}
...
\begin{align*}
\risk_{L,B}(f) = \int_{B}L(\o, f(x)) d\o.
\end{align*}
