...
Let $(\O, \A, \prob)$ be a \textbf{probability space} and $\T$ a \textbf{parameter space}. Furthermore, let $X$ be a \textbf{random variable} defined over $(\O, \T)$. Then we denote the \textbf{probability density function} of $X$ as $p(x; \t)$.
\end{definition}

Furthermore, we want to introduce parametric models. These are families of distributions that can be indexed by a finite number of parameters. Here we should note, that we want to consider the defining parameters of a probability distribution in the arguments of the underlying probability density function. This means that for example for a normally distributed random variable $X$ with mean $\m$ and variance $\s^2$ we would denote its probability density function as $p(x;\m,\s^2)$.
...
Another way to generate a family of distributions is to alter an underlying base probability density function, hence named standard probability density function. Possible alterations might be shifting or scaling the standard probability density function (or applying both alterations). Therefore, we cite a theorem from \cite[Theorem~2.1]{cinelli2021variational}, which addresses exactly this problem.
