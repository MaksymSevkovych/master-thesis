...
\begin{example}\label{ex:nn_gradients}
...
\end{align}
where $w\in\R$ is the networks weight and $b\in\R$ is the networks bias. For simplicity, we assume $b=0$.\\
...
\risk_{\loss, D} (f_\t) = \frac{1}{d} \sum_{i=1}^d \left| y_i - \f(wx_i) \right|^2.
\end{align}
Now, lets consider how the gradient of \eqref{eq:risk_expl} with regard to the parameter $\t=w$ of the neural network $w$ looks like. In mathematical notation, this means $\partial \risk_{\loss, D}(f_w)/\partial w$.\\
However, since the empirical risk function is the mean of loss functions, the gradient of the empirical risk function is the mean of the gradients of the loss functions as well. Hence, we need to compute $\partial \loss(f_w, y)/\partial w$.\\
At this point, we want to remember the chain rule of differentiation, which states
...
\frac{\partial \loss(f_w(x), y)}{\partial w} = \frac{\partial \loss(f_w(x), y)}{\partial f_w(x)}\frac{\partial f_w(x)}{\partial w}.
\end{align}
This expression we can compute easily, since $\partial f_w(x)/\partial w$ is
\begin{align*}
\frac{\partial f_w(x)}{\partial w} = \frac{\partial \f(wx)}{\partial w} = \frac{\partial \f(wx)}{\partial wx} \frac{\partial wx}{\partial w} = x\f^{\prime}(x),
\end{align*}
where the derivative of the sigmoid function is $\f^{\prime}(x) = \f(x)(1 - \f(x))$.\\
Furthermore, the second part of the right-hand side $\partial \loss(f_w(x), y)/\partial f_w(x)$ is
\begin{align*}
\frac{\partial \loss(f_w(x), y)}{\partial f_w(x)} = \frac{\partial |f_w(x) - y|^2}{\partial f_w(x)} = 2 \left(f_w(x) - y \right) = 2 \left(\f(wx) - y \right).
\end{align*}
Therefore, plugging this into equation \eqref{eq:loss_expl}, we receive
\begin{align*}
\frac{\partial \loss(f_w(x), y)}{\partial w} = 2x \f^{\prime}(x) \left(wx - y \right) = 2 \f^{\prime}(x) \left(wx^2 - yx \right).
\end{align*}
Lastly, we can insert the data samples $(x_i, y_i)\in D$ to compute the gradient of the empirical risk function.
\item Since, neural networks usually consist of more than two neurons, we now want to consider how we can generalize the neural network to more than two neurons.\\
Let $L\in\N$ and $L+1$ denote the number of layers in the neural network. Furthermore, let each layer consist of one neuron, hence for each $i=1,\ldots, L$ holds
\begin{align*}
H_i: \R &\to \R,\\
x &\mapsto \f\left(wx_i + b_i\right).
\end{align*}
For simplicity reasons, we assume again that $\f=\id$. Therefore, we can denote the neural network as
\begin{align*}
f_\t(x) = H_L \circ \ldots \circ H_1(x) = H_L(H_{L-1}(\ldots(H_1(x))\ldots) \qquad x\in\R,
\end{align*}
where $\t=(\t_1,\ldots,\t_L)$ denotes the parameters of the neural network.\\
Since our goal is to compute the gradient of the empirical risk function, we can compute the gradient of the loss, analogously to the first example.
\begin{align*}
\frac{\partial \loss(f_\t(x), y)}{\partial \t} = \frac{\partial \loss(H_L(\ldots(H_1(x)\ldots), y)}{\partial \t}.
\end{align*}
Considering the chain rule of derivation from equation \eqref{eq:chain_rule}, we receive
\begin{align*}
\frac{\partial \loss(f_\t(x), y)}{\partial \t} = \frac{\partial \loss(f_\t(x), y)}{\partial f_\t(x)} \frac{\partial f_\t(x)}{\partial \t} = \frac{\partial \loss(f_\t(x), y)}{\partial f_\t(x)}\frac{\partial H_L(x)}{\partial H_{L-1}(x)}\frac{\partial H_{L-1}(x)}{\partial H_{L-2}(x)} \ldots \frac{\partial H_1(x)}{\partial \t_1}.
\end{align*}
At this point, we want to consider $b_i=0$ for all $i=1,\ldots L$ for simplicity reasons, as well. Hence, $\t_i=w_i$ for all $i=1,\ldots L$ and the gradient simplifies to
\begin{align*}
\frac{\partial \loss(f_\t(x), y)}{\partial \t} &= \frac{\partial \loss(f_\t(x), y)}{\partial f_\t(x)}\frac{\partial H_L(x)}{\partial H_{L-1}(x)}\frac{\partial H_{L-1}(x)}{\partial H_{L-2}(x)} \ldots \frac{\partial H_2(x)}{\partial H_1(x)} \frac{\partial H_1(x)}{\partial w_1}\\
&= \frac{\partial \loss(f_\t(x), y)}{\partial f_\t(x)}\frac{\partial H_1(x)}{\partial w_1} \prod_{i=1}^{L-1} \frac{\partial H_{i+1}(x)}{\partial H_{i}(x)}.
\end{align*}
Now, considering the gradient of each layer iteratively, we receive
\begin{align*}
\frac{\partial H_1(x)}{\partial w_1} &= x,\\
\frac{\partial H_2(x)}{\partial H_1(x)} &= w_2,\\
&=\; \vdots\\
\frac{\partial H_L(x)}{\partial H_{L-1}(x)} &= w_L.
\end{align*}
Lastly, we need to consider the gradient of $\partial \loss(f_\t(x), y)/\partial f_\t(x)$, what we already know from the first example to be
\begin{align*}
\frac{\partial \loss(f_\t(x), y)}{\partial f_\t(x)} = \frac{\partial |f_\t(x)- y|^2}{\partial f_\t(x)} = 2\left( f_\t(x) - y \right)
\end{align*}
Combining the determined gradients of each layer, we receive
\begin{align*}
\frac{\partial \loss(f_\t(x), y)}{\partial \t} = 2x\left( f_\t(x) - y \right)  \prod_{i=2}^{L} w_i,
\end{align*}
which is the gradient of the loss function with the neural network as predictor.
...
As the name already suggests, the gradient descent algorithm relies on computing the gradient of the risk function in each iteration, which often is too expensive as shown in the comparatively easy architecture in the last example of Example \ref{ex:nn_gradients}.
...
Let $T\in \N$, $D$ be an arbitrary dataset and $\T\subset\R^d$ be a parameter space and $\loss$ be an arbitrary loss function. Furthermore, let $f_t(\t_t)$ be a prediction function with parameters $\t_t\in\T$ at time step $t$. Then we define the \textbf{regret} as the function
\begin{align*}
...
\begin{algorithm}[H]
Let $g_t$ as in Algorithm \ref{alg:adam}, $D$ be an arbitrary dataset and let $T\in\N$. Furthermore, let $\T$ be a parameter space.
