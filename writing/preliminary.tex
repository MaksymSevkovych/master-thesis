...
Since we now know what neural networks are, we want to discuss how to adapt them to a specific problem. This procedure is usually called training of a neural network. There are many approaches of how to train a neural network. However, most of them rely on iteratively finding the gradient - the direction of greatest ascent. In the following we want to consider a couple of popular algorithms that are used to train neural networks.

\begin{theorem}\label{theorem_gd}
Let $(\g_t)$ be a converging sequence with $\g_t \to 0$, called step size. Let further be $f:\R^n \to \R$ a continuous, convex and differentiable function. Furthermore, let $x^{(t)}$ denote the $t$-th iterate of the \textbf{gradient descent algorithm} defined by
\begin{align}
x^{(t+1)} = x^{(t)} - \g_t \nabla_x f(x^{(t)}),
\end{align}
...
Let $X, Y$ be arbitrary Banach spaces with $X\subset \R^d$ and $Y \subset \R^n$ and $d, n \in \N$, that we will refer to as input and output space, $p:X \to \R^n$ be a continuous, convex function.\\
...
\begin{align*}
\risk(p) = \int_{X\times Y} L\left(x, y, p(x) \right) d \prob(x, y).
\end{align*}
\end{definition}

\begin{corollary}
Let $f_\t:\R^d\to \R$ be a neural network with parameters $\t\in\T$ and arbitrary depth $L\in\N$ and arbitrary activation function $\f$.\\
...
\begin{align*}
\t^{(t+1)} = \t^{(t)} - \g_t \nabla_\t \risk \left(f_{\t^{(t)}(x)}\right), \qquad	x\in \R^d
\end{align*}
\end{corollary}


This is a valuable result, since this way one can iteratively optimize any convex function. Such iterative methods are powerful in numerical settings, where one could use a machine to compute the result. However, there is one problem: in many practical cases it is way to costly to compute the gradient, if the dimensionality becomes extremely large. This lead to a bunch of approaches on how to make this algorithm more efficient. This lead to the following algorithm.
