...
Then the stochastic gradient descent algorithm with update rule \eqref{eq:sgd} and posed assumptions \ref{ass:sgd} satisfies the following assertions.
\begin{enumerate}
\item \label{ass1} The empirical risk function $\risk_{\loss, \o}$ has a unique minimum at $\t^{\ast}\in\T$.
\item \label{ass2}For any $n\geq 0$ denote the \textbf{stochastic distance} as
...
\begin{proof}
Assertion~\ref{ass1}: The existence of a minimum follows from the continuity and lower boundedness of $\risk_{\loss, \o}$. The uniqueness follows from the strong convexity of $\risk_{\loss, \o}$.\\
Assertion~\ref{ass2}: It holds that
