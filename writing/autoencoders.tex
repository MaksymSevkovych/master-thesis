...
\begin{lemma}\label{composition_of_nns}
...
\begin{remark}
Let $f_e$ and $f_d$ be an encoding and a decoding neural network as in definition \ref{def_autoencoder}. Then the composition $f_d \circ f_e$ is indeed a neural network, following from lemma \ref{composition_of_nns}.
\end{remark}
...
In this section we want to train a couple of own autoencoding neural networks on the MNIST dataset - a dataset consisting of handwritten digits. We will consider some different architectures of neural networks and visualise the results in a comprehensible manner.\\
...
\begin{align*}
H_i(x) = \hat{\f}(W_ix + b_i), \qquad x \in \R^{d_i},
\end{align*}
...
\begin{align*}
H_i(x) = \hat{\f}(W_ix + b_i), \qquad x \in \R^{d_i}, i \in \{1,\ldots, L-1\},
\end{align*}
...
\caption{Linear autoencoder}\label{alg:cap}
\begin{algorithmic}[1]
\State $epochs \gets 10$
\For{$i\leq epochs$}
    \State $i \gets i-1$
\Else
    \If{$i\leq 3$}
